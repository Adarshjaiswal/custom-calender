...
const calendarScales = ['3 days', 'Week', 'Month', '3 months', 'Year'];
const eventFilter = [];
...
const displayEvents = (changeFactor, timeout) => {
    ...
    setTimeout(() => {
        ...
        for (let i = 0; i <= differenceInDays; i++) {
            let weekDay = startDate.toLocaleString('en-IN', { weekday: 'short' });
...
const changeSideCalendarMonth = (changeFactor) => {
    ...
    if (changeFactor !== 0 || appJustStarted) {
        ...
        let month = (new Date(sideCalendarDate.getTime())).toLocaleString('en-US', { month: 'long' });
    ...
    for (let i = 1; i <= sideCalendarDate.getDate(); i++) {
        ...
        sidebarCalendarElement.insertAdjacentHTML('beforeend', `
            <button class="side-calendar__day ${mostFrequentEventType}" title="Go to ${i} ${formatMonth(sideCalendarDate)}"><span translate="no">${i}</span></button>
        `);
    }
};
...
const displayEventsInList = () => {
    ...
    events.forEach((value) => {
        if (eventFilter.indexOf(value.type) === -1) {
            let month = (new Date(value.date)).toLocaleString('en-US', { month: 'long' });
            month = month[0].toUpperCase() + month.slice(1);
...
const fetchHolidays = (replaceOldHolidays) => {
    fetch(`https://holidayapi.com/v1/holidays?key=3d6be5b4-56ed-4899-a3a0-bbbc9d0c251c&country=${countryElement.value.toUpperCase()}&year=2023`)
        .then((response) => response.json())
        .then((response) => {
            if (replaceOldHolidays) {
                // Remove old holidays from events
            ...
            // Update localStorage with the new holiday data
            localStorage.setItem('holidaysSaved', '1');
            localStorage.setItem('events', JSON.stringify(events));

            if (replaceOldHolidays) {
                // Update the UI if in list mode
                if (displayEventsInListMode) {
            ...
            // Change the side calendar month to the current month
            changeSideCalendarMonth(0);
        })
        .catch(() => {
            alert('Failed to fetch holidays for this country');
        });
};
    ...
    if (eventNameElement.value !== '' && eventHoursElement.value !== '' && eventMinutesElement.value !== ''
        ...
        if (+(eventHoursElement.value) >= 0 && +(eventHoursElement.value) <= 23
            ...
            if (sameEventIndex === -1) {
                ...
                alert('An event with the same time and date already exists!');
            }
        } else {
            alert('Enter a valid time or date!');
        }
    } else {
...
const showAddEventPopup = (eventOrTask) => {
    if (!editEvent) {
        document.querySelector('.add-event__header').innerHTML = `Adding ${eventOrTask}`;
        document.querySelector('#createEvent p').innerHTML = `Create ${(eventOrTask === 'event') ? 'event' : 'task'}`;
        document.querySelector('#createEvent img').src = `{{asset('images/add.svg')}}`;
        ...
        document.querySelector('.add-event__header').innerHTML = `Editing ${eventOrTask}`;
        document.querySelector('#createEvent p').innerHTML = 'Edit Event';
        document.querySelector('#createEvent img').src = `{{asset('images/change.svg')}}`;
        document.querySelector('#deleteEvent').style.display = 'block';
    }

    document.querySelector('#eventNamePlaceholder').innerHTML = `Name ${eventOrTask}`;
    document.querySelector('#eventDescriptionPlaceholder').innerHTML = `Description ${eventOrTask}`;
    document.querySelector('#eventTypeHeader').innerHTML = `Type ${eventOrTask}`;
    document.querySelector('#eventTimeHeader').innerHTML = `Time ${eventOrTask}`;
    document.querySelector('#eventDateHeader').innerHTML = `Date ${eventOrTask}`;
    document.querySelector('#eventTimeHeader').innerHTML = `Time ${eventOrTask}`;
...
document.addEventListener('click', (event) => {
    ...
    if (calendarScaleOpened && !event.target.closest('#scale .dropdown__current')) {
        ...
        if (displayEventsInListMode) {
            // document.querySelector('#displayEventsInList img').src = `{{asset('images/calendar.svg')}}`;
            document.querySelector('#displayEventsInList').title = 'Display events on the calendar';
            ...
            // document.querySelector('#displayEventsInList img').src = `{{asset('images/list.svg')}}`;
            document.querySelector('#displayEventsInList').title = 'Display events in list view';
        ...
        showAddEventPopup((editableEvent.eventOrTask) ? 'events' : 'tasks');
